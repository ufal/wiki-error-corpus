...
# Input directory: Location of the stage2a directory 
input_dir=$1

# Output directory: stage3 output 
output_dir=$2
...
for i in $(ls); do
  ...
  qsub -S /bin/bash -wd ${output_dir} -p -200 -e /dev/null -o /dev/null  ${code_root}/run/stage3.sh ${i} ${input_dir} ${output_dir} ${code_root}
done
popd
