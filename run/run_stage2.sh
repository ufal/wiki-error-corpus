...
# Input directory: Location of the stage1 directory 
input_dir=$1
...
for i in $(find . -type f -size -16M -exec ls -1 {} \; | perl -npe 's/\.\///'); do
  ...
  qsub -S /bin/bash -wd ${output_dir} -p -200 -e /dev/null -o /dev/null  ${code_root}/run/stage2.sh ${i} ${input_dir} ${output_dir} ${code_root}
done
popd
